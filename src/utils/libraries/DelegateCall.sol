...
library DelegateCall {
    /**
     * @dev DelegateCall to the target using the given data. Revert when error occurs.
    ...
    /**
     * @dev DelegateCall to the multi target. Revert when error occurs.
        ...
        returns (bytes[] memory results)
    {
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            (bool success, bytes memory result) = _targets[i].delegatecall(_data[i]);
    ...
    /**
     * @dev DelegateCall to the target using the given data. Just return when error occurs.
     * @param _target The target address to call.
     * @param _data The data used in the call.
     */
    function callWithNoRevert(address _target, bytes memory _data) internal returns (bytes memory) {
        (, bytes memory result) = _target.delegatecall(_data);
        return result;
    }

    /**
     * @dev DelegateCall to the multi target. Just return when error occurs.
     * @param _targets The target addresses to call.
     * @param _data The data used in the call.
     */
        ...
        returns (bytes[] memory results)
    {
        results = new bytes[](_data.length);
        for (uint256 i = 0; i < _data.length; i++) {
            (, bytes memory result) = _targets[i].delegatecall(_data[i]);
            results[i] = result;
        }
    }
}
